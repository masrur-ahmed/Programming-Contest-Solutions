...
int main(){
	...
	string s;
	cin>>s;
	bool v= false ,c=false,n=false,sts=true;;
	int i=1;
	if(s[0]=='a' || s[0]=='e'|| s[0]=='i' || s[0]=='o' || s[0]=='u' )v=true;
	else if (s[0]=='n')n=true;
	else c=true;
	FOR(i,1,s.size()-1){
		
		if(n && !v && !c){
			if(s[i]=='a' || s[i]=='e'|| s[i]=='i' || s[i]=='o' || s[i]=='u' )v=true,c=false,n=false;
			else if (s[i]=='n')n=true,v=false,c=false;
			else c=true,v=false,n=false;
		}
		else if (v && !c && !n){
			if(s[i]=='a' || s[i]=='e'|| s[i]=='i' || s[i]=='o' || s[i]=='u' )v=true,c=false,n=false;
			else if (s[i]=='n')n=true,v=false,c=false;
	...
	i=s.size()-1;
	if((s[i]=='a' || s[i]=='e'|| s[i]=='i' || s[i]=='o' || s[i]=='u' || s[i]=='n')&& sts ){sts=true;}
	else sts=false;
	if(sts)pf("YES");
	else pf("NO");
