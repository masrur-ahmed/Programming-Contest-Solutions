...
int main(){
    ...
    sfl2(p[0].first,p[0].second);
    if(p[0].first>p[0].second)swap(p[0].first,p[0].second);
    for(int i = 1 ; i < n ; i++){
        sfl2(p[i].first,p[i].second);
        if(p[i].first<=p[i-1].second && p[i].second<=p[i-1].second){
            if(p[i].first>p[i].second)swap(p[i].first,p[i].second);
        }
        else if(p[i].first<p[i].second)swap(p[i].first,p[i].second);
