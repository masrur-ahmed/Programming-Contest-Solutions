 ...
 */
 int i=0,j=0,k=0,cas=0,t=0;
int main(){
	int team,mem,dqid;
	while(sf1(team)!=0 && team!=0){
		
		
		queue<int>qbyid[team],generalQ;
		map<int,int>teamList;
		REP(i,team){
			sf1(mem);
			REP(j,mem){
				sf1(k);
		  ...
		  //for (std::map<int,int>::iterator it=teamList.begin(); it!=teamList.end(); ++it)
		...
		while(true){
			...
			if(s[0]=='S')break;
				...
				cin>>mem;
				if(qbyid[teamList[mem]].size()!=0){
					qbyid[teamList[mem]].push(mem);
				}
				else {
					qbyid[teamList[mem]].push(mem);
					generalQ.push(teamList[mem]);
				}
			}
			
			else if(s[0]=='D'){
				 dqid=generalQ.front();
				 pf("%d\n",(int)qbyid[dqid].front());
				 qbyid[dqid].pop();
				 if(qbyid[dqid].size()==0)generalQ.pop();
			}
