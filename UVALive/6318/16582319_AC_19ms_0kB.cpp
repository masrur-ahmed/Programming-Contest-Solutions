...
int main(){
	int t,c,v,mx1,mx2,can1,can2;
	int priority[101][101];
	sf1(t);
	REP(i,t){
		sf2(c,v);
		REP(j,v){
		...
		for(map<int,int>::iterator it=m.begin(); it!=m.end(); ++it){
			if(it->second>mx1){
				...
				//cout<<"can"<<it->first<<endl;
		}
		//cout<<mx1<<" "<<can1<<" "<<mx2<<" "<<can2<<" "<<endl;
		if(mx1>(v/2)){
			printf("%d 1\n",can1);
			m.clear();
			continue;
		...
		REP(j,v){
			REP(k,c){
				if(priority[j][k]==can1){mx1++;break;}
				if(priority[j][k]==can2){mx2++;break;}
			}
			
		}
		if(mx1>mx2)printf("%d 2\n",can1);
		else printf("%d 2\n",can2);
